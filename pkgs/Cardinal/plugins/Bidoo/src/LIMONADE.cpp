...
void LIMONADE::loadSample() {
	...
	async_dialog_filebrowser(false, nullptr, "Load sample", [this](char* path) {
		loadSamplePath(path);
	});
#else
...
#ifdef USING_CARDINAL_NOT_RACK
	async_dialog_filebrowser(false, nullptr, "Load frame", [this](char* path) {
		loadFramePath(path);
	});
#else
	char *path = osdialog_file(OSDIALOG_OPEN, "", NULL, NULL);
...
#ifdef USING_CARDINAL_NOT_RACK
	async_dialog_filebrowser(false, nullptr, "Load PNG", [this](char* path) {
		loadPNGPath(path);
	});
#else
	char *path = osdialog_file(OSDIALOG_OPEN, "", NULL, NULL);
