 ...
 # Select internationalisation properties.
  ...
  # $ nix search wget
   environment.systemPackages = with pkgs; [
    csound  alsaLib vim git bazel_4 firefox  qjackctl pavucontrol jack_capture
   ];
  # Some programs need SUID wrappers, can be configured further or are
  # started in user sessions.
  # programs.mtr.enable = true;
  # programs.gnupg.agent = { enable = true; enableSSHSupport = true; };
  nixpkgs.overlays = [(import ../../pkgs/plymouth)];
  boot.plymouth = {
   enable = true;
...
#          greeters.pantheon.enable = lib.mkForce false;
#        };
      # startx = {
      #  enable = true;
      # };
...
#        defaultSession = "pantheon";
  #    };
   ...
   #   desktopManager = {
  #      pantheon = {enable = true;}; # <---THIS LINE!
    #  };
    #};
...
services.jack = {
  ...
  # Define a user account. Don't forget to set a password with ‘passwd’.
   users.users.orlyn = {
     isNormalUser = true;
     extraGroups = [ "sudoers" "wheel" "networkmanager" "audio" "video" "disk" "jackaudio" "realtime" ]; # Enable ‘sudo’ for the user.
     initialPassword = "loyde1029";
     shell = pkgs.ion; 
   };


users.users.root.initialPassword = "loyde1029";
